...
# Group by 'Original File Name' and calculate mean values for time taken
gb_first_time = df_first.groupby('Original File Name')['Time Taken by Brotli(s)'].mean().reset_index()
gb_second_time = df_second.groupby('Original File Name')['Time Taken by Brotli(s)'].mean().reset_index()

# Get the original file sizes
gb_first_size = df_first.groupby('Original File Name')['File Size(B)'].first().reset_index()

# Merge the time and size dataframes
merged_df = pd.merge(gb_first_time, gb_second_time, on='Original File Name', suffixes=('_1', '_2'))
merged_df = pd.merge(merged_df, gb_first_size, on='Original File Name')

# Sort by 'File Size(B)'
merged_df = merged_df.sort_values(by='File Size(B)')

# Calculate percentage change
merged_df['percentage_change'] = -(merged_df['Time Taken by Brotli(s)_1'] - merged_df['Time Taken by Brotli(s)_2']) * 100 / merged_df['Time Taken by Brotli(s)_1']
...
# Plot the time lines
ax.plot(x, merged_df['Time Taken by Brotli(s)_1'] * 1000, marker='o', label='Compressed Using Brotli Time 1 (ms)')
ax.plot(x, merged_df['Time Taken by Brotli(s)_2'] * 1000, marker='o', label='Compressed Using Brotli Time 2 (ms)')
